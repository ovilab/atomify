...
void main(void) {
    float radius = 0.5;
    float sphereRadius = 1.0; // Spheres discard any r^2 > 0.9 => r>0.95
    ...
    if(distanceToV1 < distanceToV2) {
        // v1 closer than v2
        gl_Position = cp_modelViewProjectionMatrix*vec4(v2 + da*deltaProjectedNormalized - orthogonalOnDelta*radius, 1.0);
        texCoord = vec2(-da, 1.0);
        EmitVertex();
        gl_Position = cp_modelViewProjectionMatrix*vec4(v1 - da*deltaProjectedNormalized - orthogonalOnDelta*radius, 1.0);
