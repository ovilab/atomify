...
CPFix::CPFix(QObject* parent)
    : SimulatorControl(parent)
{
    setType("Fix");
}
...
bool CPFix::copyData(LAMMPS_NS::FixAveChunk* fix, LAMMPSController* lammpsController)
{
    ...
    bool* compress = static_cast<bool*>(chunk->extract("compress", dimension));

    if (!nvalues || !nchunk || !chunk || !colextra || !compress) {
        qDebug() << "Warning, could not get values from FixAveChunk::extract.";
        return true;
    }

    if (*compress) {
        qDebug() << "Warning, we don't support ComputeChunkAtom with compress yes.";
        return true;
    }
    ...
    if (*which == BINCYLINDER) {
        int* ncplane = static_cast<int*>(chunk->extract("ncplane", dimension));
        if (*ncplane > 1)
            return true; // It is a cylinder, but more than 1 bin along the cylinder dimension. We don't support this yet, so ignore it.

        if (fix->nextvalid() == lammpsController->system->currentTimestep() + 1) {
            for (int j = 0; j < *nvalues; j++) {
                QString key = QString("Value %1").arg(j + 1);
                Data1D* data = ensureExists(key, true);
                data->setLabel(key);
                data->clear(true);

                for (int i = 0; i < *nchunk; i++) {
                    float r = fix->compute_array(i, 0);

                    int valueIndex = *colextra + 1 + j;
                    float value = fix->compute_array(i, valueIndex);
        ...
        return true;
        // return handleBinCylinder(fix, lammpsController);
    }
...
void CPFix::copyData(LAMMPSController* lammpsController)
{
    ...
    if (lmp_fix == nullptr)
        return;
    if (copyData(dynamic_cast<LAMMPS_NS::FixAveChunk*>(lmp_fix), lammpsController))
        return;
    if (copyData(dynamic_cast<LAMMPS_NS::FixAveHisto*>(lmp_fix), lammpsController))
        return;
    if (copyData(dynamic_cast<LAMMPS_NS::FixAveTime*>(lmp_fix), lammpsController))
        return;
}
