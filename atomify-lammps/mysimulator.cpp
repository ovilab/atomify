...
void MyWorker::synchronizeRenderer(Renderable *renderableObject)
{
    ...
    if(lines) {
        QVector<QVector3D> &data = lines->data;
        data.push_back(p1);
        data.push_back(p2);
    }
#else
    ...
    Lines *lines = qobject_cast<Lines*>(renderableObject);
    float maxRadiusSquared = 1.2*1.2;
    if(false && lines) {
        QVector<QVector3D> &data = lines->data;
        data.resize(0);
        if( lammps->neighbor->nlist > 0) {
            ...
            for(int ii=0; ii<inum; ii++) {
                ...
                for (int jj = 0; jj < jnum; jj++) {
                    ...
                    if(dr2 > maxRadiusSquared) continue;

                    QVector3D v1;
                    v1[0] = posi[0]  - lammps->domain->boxlo[0] - lammps->domain->prd_half[0];
                    v1[1] = posi[1]  - lammps->domain->boxlo[1] - lammps->domain->prd_half[1];
                    v1[2] = posi[2]  - lammps->domain->boxlo[2] - lammps->domain->prd_half[2];
                    QVector3D v2;
                    v2[0] = posj[0]  - lammps->domain->boxlo[0] - lammps->domain->prd_half[0];
                    v2[1] = posj[1]  - lammps->domain->boxlo[1] - lammps->domain->prd_half[1];
                    v2[2] = posj[2]  - lammps->domain->boxlo[2] - lammps->domain->prd_half[2];
                    data.push_back(v1);
                    data.push_back(v2);
                }
            }
