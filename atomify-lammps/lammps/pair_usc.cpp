...
void PairUSC::compute(int eflag, int vflag)
{
    ...
    for (ii = 0; ii < inum; ii++) {
        ...
        for (jj = 0; jj < jnum; jj++) {
            ...
            ijparam = moleculeMap[itype][jtype][jtype];
        ...
        for (jj = 0; jj < jnum - 1; jj++) {
            ...
            for (kk = jj+1; kk < jnum; kk++) {
                ...
                ktype = atomTypeMap[type[k]];
                ikparam = moleculeMap[itype][ktype][ktype];
                ijkparam = moleculeMap[itype][jtype][ktype];
...
void PairUSC::coeff(int narg, char **arg)
{
    ...
    for (int i = 1; i <= n; i++)
        for (int j = i; j <= n; j++)
            if (atomTypeMap[i] >= 0 && atomTypeMap[j] >= 0) {
                setflag[i][j] = 1;
                count++;
            }
...
void PairUSC::setup()
{
    ...
    memory->create(moleculeMap,nelements,nelements,nelements, "pair:elem2param");
